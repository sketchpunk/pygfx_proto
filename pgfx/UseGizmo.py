"""
ORIGINAL SOURNCE
https://github.com/pygfx/pygfx/blob/fa71a9369733323d9b62e39b9ef6b5408bb7a6f5/pygfx/helpers/_gizmo.py

CHANGES
- Sent to render last
- All materals have depth test off to render over objects
- Added a set of hooks to get when gizmo moves, rotates or scales
"""

import numpy as np
import pylinalg as la

from pygfx.objects import Line, Mesh
from pygfx.geometries import Geometry, sphere_geometry, cone_geometry, box_geometry
from pygfx.materials import MeshBasicMaterial, LineMaterial
...
class TransformGizmo(WorldObject):
    ...
    * Grab the spheres to rotate.
    ...
    def _create_elements(self):
        ...
        scale_uniform = Mesh(
            sphere_geo,
            MeshBasicMaterial(color=WHITE, pick_write=True, depth_test=self.allow_depth),
        )
        ...
        line_x = Line(
            line_geo,
            LineMaterial(thickness=THICKNESS, color=RED, depth_test=self.allow_depth),
        )
        line_y = Line(
            line_geo,
            LineMaterial(thickness=THICKNESS, color=GREEN, depth_test=self.allow_depth),
        )
        line_z = Line(
            line_geo,
            LineMaterial(thickness=THICKNESS, color=BLUE, depth_test=self.allow_depth),
        )
        ...
        translate_x = Mesh(
            cone_geo,
            MeshBasicMaterial(color=RED, pick_write=True, depth_test=self.allow_depth),
        )
        translate_y = Mesh(
            cone_geo,
            MeshBasicMaterial(color=GREEN, pick_write=True, depth_test=self.allow_depth),
        )
        translate_z = Mesh(
            cone_geo,
            MeshBasicMaterial(color=BLUE, pick_write=True, depth_test=self.allow_depth),
        )
    ...
    def _update_visibility(self):
        ...
        plane_normal = la.vec_transform(np.eye(3), self.world.matrix) - self.world.position
        ...
        gizmo_to_screen = self._ndc_to_screen @ self._camera.camera_matrix @ self.world.matrix
    ...
    def _handle_translate_move(self, event):
        ...
        if self.onMove:
            self.onMove(self._object_to_control.world.position.copy())
    ...
    def _handle_scale_move(self, event):
        ...
        if self.onScale:
            self.onScale(self._object_to_control.world.scale.copy())
    ...
    def _handle_rotate_move(self, event):
        ...
        if self.onRotate:
            self.onRotate(self._object_to_control.world.rotation.copy())
